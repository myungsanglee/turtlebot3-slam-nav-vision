...
**footprint**: 코스트맵이 충돌 판정에 쓰는 "로봇의 외형"이다. 처음엔 표준 burger 반경
0.105m 원으로 임시 설정했다가, **2026-09-08 실측 다각형으로 교체**했다: base_footprint(바퀴 축
중심) 기준 앞 +0.062m(카메라 전면이 가장 앞), 뒤 −0.220m, 좌우 ±0.090m 의 직사각형. 이 로봇은
낮고 길어(전장 28cm) 원 하나로는 뒤쪽이 잘려 실제로 부딪힐 수 있었다. 외형보다 작게 잡으면
부딪히고, 크게 잡으면 좁은 통로를 못 지나가므로 실측값을 그대로 쓴다.
...
1. ~~footprint 실측 교체~~ → **완료 (2026-09-08)**: 실측 직사각형 반영. 실주행에서 좁은
   통로 통과·벽 여유를 보고 inflation_radius(0.55) 와 함께 미세 조정.

...
**다음 (우선순위 순)**
1. 레포 구조 정리 (위 6번 구조로 패키지 스캐폴딩)
2. **커스텀 로봇 URDF 보정** — LDS 위치/높이 실측 반영, RealSense camera_link TF 추가
3. slam_toolbox 설정/런치 작성 (online async, 커스텀 프레임 반영)
4. RealSense 런치 (Pi 쪽) + bringup 과 함께 뜨는 통합 런치
5. Nav2 설정 (커스텀 footprint), robot_localization EKF 설정
6. SLAM + Nav2 + RealSense + RViz 통합 런치 (한 창에서 다 보기)
7. Vision AI 노드 (TensorRT 추론) 통합
